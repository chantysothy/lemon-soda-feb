...
//                        timeline = this.curateTimeLine(timeline);
    ...
    this.executeNow = function () {
        ...
        var TASK_COMPLETE = "COMPLETE"; TASK_EXECUTING = "EXECUTING";
        ...
        schedulerTaskModel.find(condition, function (err, docs) {
            ...
            if (docs.length>0) {
                ...
                for (var taskCounter = 0; taskCounter < docs.length; taskCounter++) {
                    var taskDoc = docs[taskCounter];
                    var taskInfo = taskDoc.task;
                    if (!taskDoc['status']) {
                        taskDoc['status'] = TASK_EXECUTING;
                        var callbackForScheduler = taskDoc['callback'];
                        ...
                        var executionTime = taskDoc.executeAt;
                        var timeForExecution = executionTime - Date.now();
                        //taskDoc.executeAt to be replaced by the next suitable task
                        //if no more task remove the task from the list
                        this._taskHandle = setTimeout(function () {
                            try {
                                var callbackForScheduler = taskDoc['callback'];
                                ...
                                taskDoc['status'] = TASK_COMPLETE;
                                //update executeAt with the next value in array
                                taskDoc.save(function (err, docs) {
                                    if (err) {
                                        throw new Error("ERROR WHILE UPDATING TASK STATUS. ERROR DETAILS - " + JSON.stringify(err));
                                    }//if (err) {
                                });//docs[taskCounter].save(function (err, docs) {
